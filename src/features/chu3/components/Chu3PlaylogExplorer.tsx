      ...
      <SectionCard className="min-w-0">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <Typography.Title level={4} style={{ margin: 0 }}>{texts.playlogExplorer.title}</Typography.Title>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button size="small" disabled={page >= totalPage} onClick={() => setPage((x) => Math.min(totalPage, x + 1))}>
              {texts.common.nextPage}
            </Button>
          </div>
        </div>

        {error ? <Text className="text-app-danger mt-4 text-sm">{error}</Text> : null}
        ...
        <div className="mt-4 flex flex-col gap-3">
          {loading && !records.length ? (
            ...
            list.map(({ row, meta, key }) => {
              ...
              return (
                <button
                  ...
                  className={`border-app-line rounded-xl border p-3 text-left transition-colors ${
                    key === picked?.key ? 'bg-app-fill' : 'bg-app-recessed hover:bg-app-tint'
                  }`}
                >
            ...
            <Text className="text-app-subtle py-6 text-center">{texts.playlogExplorer.noMatches}</Text>
          )}
        </div>
      </SectionCard>

      <SectionCard className="min-w-0 xl:self-start">
        {row ? (
          <>
            <div className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <Typography.Title level={4} className="mt-1 break-words text-[1.35rem]" style={{ margin: 0 }}>
            ...
            <div className="border-app-line mt-4 overflow-hidden rounded-xl border">
              {infoRows.map((item) => (
                <div key={item.label} className="border-app-line grid grid-cols-[92px_1fr] gap-3 border-b px-3 py-2 text-sm sm:grid-cols-[130px_1fr] last:border-b-0">
                  <div className="text-app-subtle">{item.label}</div>
                  <div className="font-medium text-app-default">{item.value}</div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-xs">
              ...
              {row.isFullCombo ? <span className="rounded-full bg-app-fill px-2 py-1 text-app-brand">FC</span> : null}
              {row.isClear ? <span className="rounded-full bg-app-fill px-2 py-1">{texts.playlogExplorer.clear}</span> : null}
              {row.isFreeToPlay ? <span className="rounded-full bg-app-fill px-2 py-1">{texts.playlogExplorer.freePlay}</span> : null}
              {row.track != null ? <span className="rounded-full bg-app-fill px-2 py-1">{texts.playlogExplorer.trackBadge(row.track)}</span> : null}
            </div>

            <div className="mt-5">
              <Text className="mb-2 text-sm font-medium">{texts.playlogExplorer.judgeCounts}</Text>
              <div className="border-app-line overflow-hidden rounded-xl border">
                {judgeRows.map((item) => (
                  <div key={item.label} className="border-app-line grid grid-cols-[92px_1fr] gap-3 border-b px-3 py-2 text-sm sm:grid-cols-[130px_1fr] last:border-b-0">
                    <div className="text-app-subtle">{item.label}</div>
                    <div className="font-medium text-app-default">{item.value ?? '—'}</div>
            ...
            <div className="mt-5">
              <Text className="mb-2 text-sm font-medium">{texts.playlogExplorer.laneRates}</Text>
              <div className="border-app-line overflow-hidden rounded-xl border">
                {rateRows.map((item) => (
                  <div key={item.label} className="border-app-line grid grid-cols-[92px_1fr] gap-3 border-b px-3 py-2 text-sm sm:grid-cols-[130px_1fr] last:border-b-0">
                    <div className="text-app-subtle">{item.label}</div>
                    <div className="font-medium text-app-default">{fmtRate(item.value)}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Text className="text-app-subtle">{texts.playlogExplorer.pickPlay}</Text>
        )}
      </SectionCard>
    </div>
  )
