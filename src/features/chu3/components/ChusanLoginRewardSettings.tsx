  ...
  return (
    <div className="mt-4 flex max-w-md flex-col gap-4">
      <label className="flex flex-col gap-1">
        ...
        <Select
          value={ticketValue}
          options={Object.entries(ticketItems).map(([value, label]) => ({ value, label }))}
          onChange={(value) => setTicketValue(String(value ?? NONE_VALUE))}
        />
      </label>

      <div>
        <Button size="small" disabled={saving} onClick={() => void saveRewards()}>
