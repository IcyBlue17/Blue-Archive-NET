...
import { Chu3AppearanceSettings } from "@/features/chu3/components/Chu3AppearanceSettings";
import { ChusanLoginRewardSettings } from "@/features/chu3/components/ChusanLoginRewardSettings";
...
/**
 * 全国对战服务器的可选卡片。原来是拿 kumo 的 buttonVariants 套一个按钮的皮，
 * antd 没有等价物，直接用设计令牌写清楚——选中态就是一圈品牌色描边。
 */
function optionCardClass(active: boolean): string {
  const base =
    'border-app-line bg-app-base hover:bg-app-fill-hover flex h-auto w-full cursor-pointer ' +
    'flex-col items-stretch gap-1 rounded-lg border p-4 text-left transition-colors'
  return active ? `${base} ring-app-brand ring-2` : base
}
...
export function ChusanExtraSettings({
  ...
  return (
    <div className="flex flex-col gap-8">
      {err ? (
        <Text className="text-app-danger text-sm">{err}</Text>
      ) : null}
      ...
      <section>
        ...
        <RouterLink to="/team">
          <Button size="small">{copy.chusanExtra.openTeamPage}</Button>
        </RouterLink>
      </section>
      ...
      <div className="mt-2">
        <RouterLink to="/collectibles" className="text-app-brand text-sm font-medium text-app-brand">
      ...
      <section>
        ...
        <div className="flex max-w-xl flex-col gap-3">
          <label className="flex flex-col gap-1">
            <Text className="text-sm">{copy.chusanExtra.lxnsToken}</Text>
            <Input
              type="password"
              value={lxnsToken}
          ...
          <label className="flex flex-col gap-1">
            <Text className="text-sm">{copy.chusanExtra.lxnsFriendCode}</Text>
            <Input
        ...
        {lxnsResult ? (
          <div className="border-app-line mt-4 rounded-xl border p-4">
            ...
            {lxnsResult.warnings.length ? (
              <div className="mt-3">
                <Text className="mb-1 font-medium text-app-warning">
                  {copy.chusanExtra.lxnsWarnings}
                </Text>
                <div className="space-y-1">
                  {lxnsResult.warnings.map((one, idx) => (
                    <Text key={`${idx}-${one}`} className="text-app-subtle text-sm">
                      {one}
                    </Text>
                  ))}
                </div>
      ...
      <Modal
        ...
        title={copy.chusanExtra.matchingDialogTitle}
        footer={null}
        centered
        destroyOnHidden
        width={{ xs: '100%', sm: '92%', lg: 768 }}
      >
        <div>
          <div>
            <div className="flex flex-wrap gap-3">
              {CHU3_MATCHINGS.map((opt) => {
                ...
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  className={optionCardClass(!custom && matchingUrl === opt.matching)}
                ...
                >
                  ...
                  <div className="text-app-brand mt-2 text-xs">
                    <Typography.Link
                      href={opt.ui}
                      target="_blank"
                      rel="noreferrer"
                  ...
                  <div className="text-app-subtle mt-2 text-xs">
                    {(preset?.coop ?? []).join(" / ")}
                  </div>
                </div>
                )
              ...
              <div
                role="button"
                tabIndex={0}
                className={optionCardClass(custom)}
                onClick={clickCustom}
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") {
                    e.preventDefault();
              ...
              >
                ...
                <p className="text-app-subtle mt-1 text-xs">
