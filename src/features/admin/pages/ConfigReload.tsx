...
import * as configReloadApi from '@/features/admin/api/configReload'
...
function ValueRow({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="grid grid-cols-[minmax(0,190px)_1fr] gap-3 border-b border-app-line px-3 py-2 text-sm last:border-b-0">
      <div className="text-app-subtle">{label}</div>
      <div className="break-all font-medium text-app-default">{value}</div>
    </div>
  )
}
      ...
      {result ? (
        <>
          ...
          <AdminSection title={texts.admin.configReload.reloadedPrefixesTitle} bodyClassName="mt-4">
            <div className="flex flex-wrap gap-2">
              {result.reloadedPrefixes.map((prefix) => (
                <span
                  key={prefix}
                  className="rounded-md bg-app-fill px-2 py-1 text-xs text-app-default"
                >
                  {prefix}
                </span>
              ))}
          ...
          <AdminSection title={texts.admin.configReload.rawResponse} bodyClassName="mt-4">
            <pre className="max-h-96 overflow-auto rounded-xl border border-app-line bg-app-base px-3 py-3 text-xs leading-6">
