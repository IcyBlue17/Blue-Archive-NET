...
import * as api from '@/features/admin/api/chusanRanking'
import type { Chu3RankingEntry } from '@/features/admin/api/chusanRanking'
...
const emptyForm: RankingForm = { musicId: '', point: '', sortOrder: '', enable: true }

function formToWrite(form: RankingForm): api.Chu3RankingWrite | null {
...
function rowToForm(row: Chu3RankingEntry): RankingForm {
  return {
...
export function AdminChusanRankingPage() {
  const texts = useAppTexts()
  const t = texts.admin.chusanRanking
  const toast = useToast()
  const [type, setType] = useState<1 | 2>(1)
  const [err, setErr] = useState<string | null>(null)

  const rankingQuery = useQuery({
    queryKey: qk.adminChusanRanking(type),
    queryFn: () => api.listRanking(type),
  })

  const musicQuery = useQuery({
    queryKey: qk.gameAllMusic('chu3'),
    queryFn: () => dataApi.allMusic('chu3'),
    retry: 0,
  })
  ...
  function startEdit(row: Chu3RankingEntry) {
    setEditingId(row.id)
  ...
  async function toggleEnable(row: Chu3RankingEntry) {
    setErr(null)
    try {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <AdminSection title={t.newEntry} bodyClassName="mt-4 flex flex-col gap-3">
        ...
        <Checkbox className="text-sm" checked={form.enable} onChange={(e) => setForm({ ...form, enable: e.target.checked })}>
      ...
      <AdminSection title={type === 1 ? t.currentPeriod : t.previousPeriod} bodyClassName="mt-4">
        <div className="border-app-line bg-app-recessed text-app-subtle mb-3 rounded-md border px-3 py-2 text-xs">
          {t.effectHint}
        </div>
        ...
        {rankingQuery.isLoading ? (
          ...
          <ResponsiveTable<Chu3RankingEntry>
            rowKey={(row) => row.id}
            dataSource={rows}
            columns={[
              { title: t.sortOrder, dataIndex: 'sortOrder', key: 'sortOrder', width: 88, fixed: 'left' },
              {
                ...
                key: 'music',
                render: (_, row) => {
                  const jacket = musicJacketUrl('chu3', row.musicId)
