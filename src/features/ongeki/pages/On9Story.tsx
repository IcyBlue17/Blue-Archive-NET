...
export function On9StoryPage() {
  ...
  return (
    <div className="pb-10">
      ...
      {err || loadErr ? <Text className="text-app-danger mb-4 text-sm">{err ?? loadErr}</Text> : null}

      <section>
        <h2 className="text-app-default mb-1 text-lg font-semibold">{texts.story.chaptersTitle}</h2>
        <Text className="text-app-subtle mb-3 block text-sm">{texts.story.chaptersHint}</Text>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="flex max-w-sm flex-1 flex-col gap-1">
            <span className="text-app-subtle text-xs">{texts.story.searchLabel}</span>
            <Input
              className="h-10"
              value={chapterSearch}
          ...
          <label className="flex w-full flex-col gap-1 sm:w-48">
            <span className="text-app-subtle text-xs">{texts.story.typeFilterLabel}</span>
            <select
              value={chapterTypeFilter}
              onChange={(e) => setChapterTypeFilter(e.target.value)}
              className="border-app-line bg-app-base h-10 rounded-lg border px-3 text-sm text-app-default"
            >
        ...
        {chapterRows.length === 0 ? (
          <Text className="text-app-subtle text-sm">{texts.story.noChapters}</Text>
        ) : filteredChapterRows.length === 0 ? (
          <Text className="text-app-subtle text-sm">{texts.story.noMatches}</Text>
        ) : (
          <>
            <div className="border-app-line overflow-hidden rounded-lg border">
              {chapterPageSlice.map((row) => {
                ...
                return (
                  <div
                    key={row.chapterId}
                    className="border-app-line flex flex-wrap items-center gap-3 border-b p-3 last:border-b-0"
                  >
                    <div className="text-app-subtle w-14 shrink-0 text-xs">#{row.chapterId}</div>
                    <div className="text-app-default min-w-[160px] flex-1 truncate text-sm font-medium">
                      {row.name}
                    </div>
                    {row.type ? (
                      <span className="bg-app-tint text-app-subtle shrink-0 rounded-md px-2 py-1 text-xs">
                        {row.type}
                      </span>
                    ) : null}
                    {!row.hasProgress ? (
                      <span className="bg-app-warning-tint text-app-warning shrink-0 rounded-md px-2 py-1 text-xs">
                        {texts.story.notStarted}
                      </span>
                    ) : null}
                    <label className="flex items-center gap-1 text-xs">
                      <span className="text-app-subtle">{texts.story.jewelCountLabel}</span>
                      <Input
                        className="h-9 w-20"
                        type="number"
                        min={0}
                    ...
                    <LabeledSwitch size="small" label={texts.story.storyWatchedLabel} checked={isStoryWatched} onChange={(on) =>
                        setChapterDraft((m) => ({ ...m, [row.chapterId]: { jewelCount, isStoryWatched: on, isClear } }))
                      } />
                    <LabeledSwitch size="small" label={texts.story.clearedLabel} checked={isClear} onChange={(on) =>
                        setChapterDraft((m) => ({
                          ...m,
                    ...
                    {dirty ? (
                      <Button size="small" disabled={busy} onClick={() => void saveChapter(row)}>
                        {texts.common.save}
                      </Button>
                    ) : null}
                  </div>
                ...
                {chapterPageItems.map((item, idx) =>
                  item === 'ellipsis' ? (
                    ...
                    <Button
                      key={item}
                      size="small"
                      type={item === safeChapterPage ? 'primary' : 'default'}
      ...
      <section className="mt-8">
        <h2 className="text-app-default mb-1 text-lg font-semibold">{texts.story.storiesTitle}</h2>
        <Text className="text-app-subtle mb-3 block text-sm">{texts.story.storiesHint}</Text>
        {storyRows.length === 0 ? (
          <Text className="text-app-subtle text-sm">{texts.story.noStories}</Text>
        ) : (
          <div className="border-app-line overflow-hidden rounded-lg border">
            {storyRows.map((row) => {
              const d = storyDraft[row.storyId]
              ...
              return (
                <div
                  ...
                  className="border-app-line flex flex-wrap items-center gap-3 border-b p-3 last:border-b-0"
                >
                  <div className="text-app-subtle w-14 shrink-0 text-xs">#{row.storyId}</div>
                  <div className="text-app-default min-w-[160px] flex-1 truncate text-sm font-medium">
                    {row.name || texts.story.storyFallbackName(row.storyId)}
                  </div>
                  <label className="flex items-center gap-1 text-xs">
                    <span className="text-app-subtle">{texts.story.lastChapterLabel}</span>
                    <Input
                      className="h-9 w-24"
                      type="number"
                      min={0}
                  ...
                  {dirty ? (
                    <Button size="small" disabled={busy} onClick={() => void saveStory(row)}>
                      {texts.common.save}
                    </Button>
                  ) : null}
                </div>
      ...
      <section className="mt-8">
        <h2 className="text-app-default mb-1 text-lg font-semibold">{texts.story.memoryTitle}</h2>
        <Text className="text-app-subtle mb-3 block text-sm">{texts.story.memoryHint}</Text>
        <div className="mb-4">
          <Button
        ...
        {memoryRows.length === 0 ? (
          <Text className="text-app-subtle text-sm">{texts.story.noMemoryChapters}</Text>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2">
            {memoryRows.map((row) => {
              ...
              return (
                <div
                  key={row.chapterId}
                  className="border-app-line bg-app-base flex flex-col gap-2 rounded-lg border p-3 shadow-sm"
                >
                  <div className="flex items-center gap-2">
                    <div className="text-app-default text-sm font-medium">{row.name}</div>
                    {row.type ? (
                      <span className="bg-app-tint text-app-subtle rounded-md px-2 py-1 text-xs">{row.type}</span>
                    ) : null}
                  </div>
                  <div className="flex flex-wrap gap-3">
                    ...
                    <label className="flex items-center gap-1 text-xs">
                      <span className="text-app-subtle">{texts.story.gaugeNumLabel}</span>
                      <Input
                        className="h-9 w-20"
                        type="number"
                        min={0}
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-2">
                    <LabeledSwitch size="small" label={texts.story.dialogWatchedLabel} checked={isDialogWatched} onChange={(on) => setPatch({ isDialogWatched: on })} />
                    <LabeledSwitch size="small" label={texts.story.storyWatchedLabel} checked={isStoryWatched} onChange={(on) => setPatch({ isStoryWatched: on })} />
                    <LabeledSwitch size="small" label={texts.story.bossWatchedLabel} checked={isBossWatched} onChange={(on) => setPatch({ isBossWatched: on })} />
                    <LabeledSwitch size="small" label={texts.story.clearedLabel} checked={isClear} onChange={(on) => setPatch({ isClear: on })} />
                    <LabeledSwitch size="small" label={texts.story.endingWatchedLabel} checked={isEndingWatched} onChange={(on) => setPatch({ isEndingWatched: on })} />
                  </div>
                  {dirty ? (
                    <Button size="small" disabled={busy} onClick={() => void saveMemory(row)}>
                      {texts.common.save}
                    </Button>
                  ) : null}
                </div>
