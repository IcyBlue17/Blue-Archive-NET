...
export function On9CollectiblesPage() {
  ...
  return (
    <div className="pb-10">
      ...
      <section>
        <h2 className="text-app-default mb-3 text-lg font-semibold">{texts.collectibles.equipped}</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {displayRows.map((row) => {
                ...
                <div className="flex flex-1 flex-col gap-3 p-3">
                  <div>
                    ...
                    {!unlockAll && emptyUnlocks && cur === 0 ? (
                      <div className="text-app-subtle mt-1 text-xs">{texts.collectibles.nothingUnlocked}</div>
                    ) : null}
                  </div>
                  {/* 预览区始终 flex-1，把「更改」按钮顶到卡片底部，同行按钮才齐平；图片不再套灰框 */}
                  <div
                    className={`flex flex-1 items-center justify-center overflow-hidden ${
                      isWidePreview ? 'min-h-[88px]' : hasImg ? 'min-h-[180px]' : 'min-h-[48px]'
                    }`}
                  >
                    {textOnly ? (
                      <div className="border-app-line bg-app-fill text-app-subtle flex w-full items-center justify-center rounded-md border px-2 py-2 text-center text-xs font-semibold leading-snug">
                        {name}
                      </div>
                    ) : img ? (
                      <img
                  ...
                  <Button className="mt-auto" size="small" type="primary" onClick={() => openModal(row.field)}>
      ...
      <section className="mt-8">
        <h2 className="text-app-default mb-1 text-lg font-semibold">{texts.collectibles.intimacyTitle}</h2>
        <Text className="text-app-subtle mb-3 block text-sm">{texts.collectibles.intimacyHint}</Text>
        {intimacyRows.length === 0 ? (
          <Text className="text-app-subtle text-sm">{texts.collectibles.noIntimacyCharas}</Text>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 p-3">
                    <div className="border-app-line bg-app-recessed h-16 w-12 shrink-0 overflow-hidden rounded-md border">
                      {img ? (
                        <img
                          src={img}
                          crossOrigin={imgCross(img)}
                    ...
                    <div className="min-w-0">
                      <div className="text-app-default truncate text-sm font-medium">{name}</div>
                      <div className="text-app-subtle text-xs">
                        {texts.collectibles.intimacyCurrent(row.intimateLevel)}
                      </div>
                    </div>
                  </div>
                  <div className="border-app-line flex items-center gap-2 border-t px-3 py-2.5">
                    <Input
                      className="h-9 w-24"
                      type="number"
                      min={0}
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="border-app-line flex flex-col gap-2 border-t px-3 py-2.5">
                        <label className="flex items-center gap-2 text-xs">
                          <span className="text-app-subtle w-16 shrink-0">
                          ...
                          <select
                            ...
                            className="border-app-line bg-app-base h-9 flex-1 rounded-lg border px-2 text-sm text-app-default"
                          >
                        ...
                        {costumeLoadFailed && costumeOptions.length === 0 ? (
                          <div className="text-app-subtle text-xs">{texts.collectibles.costumeUnavailable}</div>
                        ) : null}
                        <label className="flex items-center gap-2 text-xs">
                          <span className="text-app-subtle w-16 shrink-0">
      ...
      <section className="mt-8">
        <h2 className="text-app-default mb-1 text-lg font-semibold">{texts.collectibles.cardTrainingTitle}</h2>
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="flex max-w-sm flex-1 flex-col gap-1">
            <span className="text-app-subtle text-xs">{texts.collectibles.searchLabel}</span>
            <Input
              className="h-10"
              value={cardSearch}
        ...
        {cardRows.length === 0 ? (
          <Text className="text-app-subtle text-sm">{texts.collectibles.noCardsOwned}</Text>
        ) : (
          <>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {cardPageSlice.map((row) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 p-3">
                      <div className="border-app-line bg-app-recessed h-16 w-16 shrink-0 overflow-hidden rounded-md border">
                        {img ? (
                          <img
                            src={img}
                            crossOrigin={imgCross(img)}
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-app-subtle text-xs">{stateLabel}</div>
                      </div>
                    </div>
                    <div className="border-app-line flex flex-col gap-2 border-t px-3 py-2.5">
                      <label className="flex items-center gap-2 text-xs">
                        <span className="text-app-subtle w-14 shrink-0">
                      ...
                      {!meta?.skillId && !meta?.choKaikaSkillId ? (
                        <div className="text-app-subtle text-xs">{currentSkillName ?? '—'}</div>
                      ) : null}
                      <div className="flex gap-2">
                        <Button size="small" disabled={busy || maxed} onClick={() => void maxOneCard(row)}>
                          {maxed ? texts.collectibles.cardAlreadyMaxed : texts.collectibles.maxOneCard}
                        </Button>
                        <Button size="small" disabled={busy} onClick={() => toggleCardEdit(row)}>
                      ...
                      {cardEditOpen.has(row.cardId)
                        ? (() => {
                            ...
                            return (
                              <div className="border-app-line mt-1 flex flex-col gap-2 border-t border-dashed pt-2">
                                <label className="flex items-center gap-2 text-xs">
                                  <span className="text-app-subtle w-24 shrink-0">
                                ...
                                <LabeledSwitch size="small" label={texts.collectibles.cardEditKaikaLabel} checked={d.kaika} onChange={(on) =>
                                    setField({ kaika: on, choKaika: on ? d.choKaika : false })
                                  } />
                                <LabeledSwitch size="small" label={texts.collectibles.cardEditChoKaikaLabel} checked={d.choKaika} onChange={(on) => setField({ choKaika: on, kaika: on ? true : d.kaika })} />
                                {!valid ? (
                                  <div className="text-app-danger text-xs">
                                    {texts.collectibles.cardEditInvalidRange}
                                  </div>
                                ) : null}
                                <div className="flex gap-2">
                                  <Button size="small" disabled={!valid || busy} onClick={() => void saveCardEdit(row)}>
                                    {texts.common.save}
                                  </Button>
                                  <Button size="small" disabled={busy} onClick={() => toggleCardEdit(row)}>
                                    {texts.common.cancel}
            ...
            {cardTotalPages > 1 ? (
              <div className="mt-4 flex flex-wrap items-center justify-center gap-1">
                ...
                {cardPageItems.map((item, idx) =>
                  item === 'ellipsis' ? (
                    <span key={`ce-${idx}`} className="text-app-subtle px-2 text-sm">
                      …
                    </span>
                  ) : (
                    <Button
                      key={item}
                      size="small"
                      type={item === safeCardPage ? 'primary' : 'default'}
      ...
      <section className="mt-8">
        <h2 className="text-app-default mb-1 text-lg font-semibold">{texts.collectibles.deckTitle}</h2>
        <Text className="text-app-subtle mb-3 block text-sm">{texts.collectibles.deckHint}</Text>
        {deckRows.length === 0 ? (
          <Text className="text-app-subtle text-sm">{texts.collectibles.deckEmpty}</Text>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {deckRows.map((deck) => {
              ...
              return (
                <div
                  key={deck.deckId}
                  className="border-app-line bg-app-base flex flex-col gap-3 overflow-hidden rounded-lg border p-3 shadow-sm"
                >
                  <div className="text-app-default text-sm font-semibold">
                  ...
                  {slots.map(([cardId, setSlot], idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-2">
                        <div className="border-app-line bg-app-recessed h-12 w-12 shrink-0 overflow-hidden rounded-md border">
                          {img ? (
                            <img
                              src={img}
                              crossOrigin={imgCross(img)}
                        ...
                        <label className="flex min-w-0 flex-1 flex-col gap-1">
                          <span className="text-app-subtle text-xs">{texts.collectibles.deckSlot(idx + 1)}</span>
                          <select
                            value={cardId}
                            onChange={(e) => setSlot(parseInt(e.target.value, 10) || 0)}
                            className="border-app-line bg-app-base h-9 rounded-lg border px-2 text-sm text-app-default"
                          >
                            <option value={0}>{texts.common.empty}</option>
                  ...
                  {hasDuplicate ? (
                    <div className="text-app-danger text-xs">{texts.collectibles.deckDuplicateWarning}</div>
                  ) : null}
                  {dirty ? (
                    <Button
                      size="small"
      ...
      {/* 与 chu3 一致：交给 antd Modal，动画 / ESC / 焦点陷阱 / 滚动锁定都由它负责 */}
      <Modal
          ...
          <>
            <div className="border-app-line bg-app-base shrink-0 border-b px-4 py-3">
              ...
              >
                ...
                {activeRow.field === 'cardId' ? (
                  <>
                    <label className="flex flex-col gap-1">
                      <span className="text-app-subtle text-xs">{texts.collectibles.character}</span>
                      <select
                        value={charaFilter}
                    ...
                    <label className="flex flex-col gap-1">
                      <span className="text-app-subtle text-xs">{texts.collectibles.rarity}</span>
                      <select
                        value={rarityFilter}
            ...
            <div className="bg-app-recessed min-h-0 flex-1 overflow-y-auto px-4 py-4">
              {filteredOptions.length === 0 ? (
                <Text className="text-app-subtle text-sm">{texts.collectibles.noMatches}</Text>
              ) : (
                <div
                  className={
                    ...
                    return (
                      <Button
                        ...
                        className={`border-app-line !bg-app-base !text-app-default h-auto min-h-0 w-full flex-col items-stretch gap-0 overflow-hidden rounded-xl border p-0 text-left shadow-sm transition-colors hover:border-app-brand ${
                          isEquipped ? 'ring-app-brand ring-2 ring-offset-2 ring-offset-app-base' : ''
                        }`}
                      >
                        ...
                        {showOwnBadge ? (
                          <div className="border-app-line border-b px-3 py-2">
                            <span
                        ...
                        {textOnly || !hasImg ? null : (
                          <div
                            className={`flex items-center justify-center p-2 ${
                              isWidePreview ? 'min-h-[100px]' : 'min-h-[200px]'
                            }`}
                          >
                            {img ? (
                              <img
                                ...
                                className={
                                  isWidePreview
                                    ...
                                    : 'max-h-[min(260px,100%)] max-w-full object-contain'
                                }
                                loading="lazy"
                              />
                            ) : null}
