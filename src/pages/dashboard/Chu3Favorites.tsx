...
export function Chu3FavoritesPage() {
  ...
  return (
    <div className="pb-10">
      ...
      <Text DANGEROUS_className="text-kumo-subtle mb-4 block text-sm">
      ...
      <Tabs
        ...
        value={tab}
        onValueChange={(value) => setTab(value as FavTab)}
      />

      {listErr ? <Text DANGEROUS_className="text-kumo-danger mb-4 text-sm">{listErr}</Text> : null}

      <LayerCard className="p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <LayerCard.Secondary>{texts.chu3Favorites.pinnedSection}</LayerCard.Secondary>
        ...
        {favorites.loading ? (
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {pinnedIds.map((id) => {
              ...
              return (
                <div key={id} className="border-kumo-line flex items-center gap-3 rounded-xl border p-3">
                  <div className="bg-kumo-recessed flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-lg">
                    {image ? (
                      <img
                        src={image}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-kumo-subtle truncate text-xs">{row?.sub || `ID ${id}`}</div>
                  </div>
                  <Button
                    size="sm"
                    variant="secondary"
      ...
      <LayerCard className="mt-6 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <LayerCard.Secondary>{texts.chu3Favorites.addSection}</LayerCard.Secondary>
          <Input
            className="sm:w-72"
            value={key}
        ...
        {full ? (
          <Text DANGEROUS_className="text-kumo-warning mt-2 block text-sm">
        ...
        {sourceLoading ? (
          ...
          <>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {pageRows.map((row) => (
                <div key={row.id} className="border-kumo-line flex items-center gap-3 rounded-xl border p-3">
                  <div className="bg-kumo-recessed flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-lg">
                    {row.image ? (
                      <img
                        src={row.image}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-kumo-subtle truncate text-xs">{row.sub}</div>
                  </div>
                  <Button
                    size="sm"
                    variant="primary"
